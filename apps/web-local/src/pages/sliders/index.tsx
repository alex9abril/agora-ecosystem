    ...
    return (
      <LocalLayout>
        ...
        <div className="p-6 max-w-7xl mx-auto">
          ...
          <SkeletonCardList count={6} className="sm:grid-cols-2 lg:grid-cols-3" />
        </div>
      </LocalLayout>
    );
  }
        ...
        {selectedContext && (
          <>
            ...
            {slidersLoading ? (
              ...
              <div className="text-center py-16 bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No hay sliders configurados</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sliders.map((slider) => (
                  <div
                    key={slider.id}
                    className="bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200 group"
                  >
                    {/* Preview del slider */}
                    <div
...
function ContextCard({ type, title, description, icon, stats, onClick, isSelected }: ContextCardProps) {
  return (
    <button
      onClick={onClick}
      className={`relative rounded-xl border-2 p-6 text-left transition-all duration-200 hover:shadow-lg ${
        isSelected
          ? 'border-black dark:border-white shadow-md bg-white dark:bg-neutral-800'
    ...
    >
      <div className="flex items-start gap-4">
        <div className={`flex-shrink-0 p-3 rounded-lg transition-colors ${
          isSelected ? 'bg-black dark:bg-white text-white dark:text-black' : 'bg-gray-100 dark:bg-neutral-700 text-gray-700 dark:text-gray-300'
        }`}>
          {icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className={`text-lg font-bold ${
              isSelected ? 'text-black dark:text-white' : 'text-gray-900 dark:text-gray-100'
            }`}>
              {title}
            </h3>
            {isSelected && (
          ...
          <div className="flex gap-4">
            {stats.map((stat, index) => (
              <div key={index} className="flex flex-col">
                ...
                <span className="text-base font-semibold text-gray-900 dark:text-gray-100">{stat.value}</span>
              </div>
            ))}
          </div>
        </div>
