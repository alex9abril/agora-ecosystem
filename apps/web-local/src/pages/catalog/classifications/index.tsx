...
import { productClassificationsService, ProductClassification } from '@/lib/product-classifications';
...
export default function ProductClassificationsPage() {
  ...
  const [formState, setFormState] = useState<ClassificationFormState>({
    name: '',
    slug: '',
  });
  ...
  const loadClassifications = async () => {
    ...
    try {
      setLoading(true);
      setPageError(null);
      const response = await productClassificationsService.list(selectedBusiness.business_id);
      setClassifications(response.data);
    } catch (error: any) {
      setPageError(error?.message || 'No se pudieron cargar las clasificaciones');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (item: ProductClassification) => {
    setEditing(item);
    setFormState({
      name: item.name,
      slug: item.slug,
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedBusiness?.business_id) {
      setFormError('Selecciona una sucursal para guardar la clasificación');
      return;
    }
    ...
    try {
      ...
      setFormError(null);
      if (editing) {
        await productClassificationsService.update(editing.id, {
          name: payload.name,
          slug: payload.slug,
        });
      } else {
        await productClassificationsService.create({
          ...payload,
      ...
      setFormError(error?.message || 'No se pudo guardar la clasificación');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <LocalLayout>
      <Head>
        <title>Clasificaciones | LOCALIA Local</title>
      </Head>

      <div className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-lg font-semibold text-gray-900">Clasificaciones de productos</h1>
            <p className="text-sm text-gray-600">
              Gestiona etiquetas internas para la sucursal {branchName}.
            </p>
          </div>
          <button
            type="button"
        ...
        {!selectedBusiness?.business_id ? (
          <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Selecciona una sucursal para gestionar las clasificaciones de productos.
          </div>
        ) : (
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-3 text-sm text-gray-600">Cargando clasificaciones...</span>
              </div>
            ) : classifications.length === 0 ? (
              <div className="px-6 py-10 text-center text-sm text-gray-500">
                No hay clasificaciones para esta sucursal. Crea la primera para comenzar.
              </div>
            ) : (
