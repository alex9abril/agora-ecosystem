...
export type KarlopayPaymentWebhookExecutionInsert = {
  httpMethod: string;
  path: string;
...
@Injectable()
export class KarlopayPaymentWebhookExecutionLogService {
  private readonly logger = new Logger(KarlopayPaymentWebhookExecutionLogService.name);

  async record(row: KarlopayPaymentWebhookExecutionInsert): Promise<void> {
    if (!dbPool) {
      return;
    }
    try {
      await dbPool.query(
        `INSERT INTO bitacora.karlopay_payment_webhook_executions (
      ...
      this.logger.warn(`No se pudo escribir bitácora karlopay webhook: ${msg}`);
    }
  }
}
