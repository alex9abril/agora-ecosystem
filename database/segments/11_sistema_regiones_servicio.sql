-- ============================================================================
-- SEGMENTO 11: SISTEMA DE REGIONES DE SERVICIO
-- ============================================================================
...
-- 
-- Dependencias: 
--   - PostGIS debe estar habilitado (se habilita automáticamente al inicio)
--   - Tabla core.businesses (Segmento 01)
-- Orden de ejecución: 11
-- ============================================================================

-- ============================================================================
-- HABILITAR POSTGIS (REQUERIDO PARA GEOMETRÍA)
-- ============================================================================
-- ⚠️ IMPORTANTE: Este script REQUIERE PostGIS habilitado
-- 
-- ANTES DE EJECUTAR ESTE SCRIPT:
-- 1. Ve a Supabase Dashboard > Database > Extensions
-- 2. Busca "postgis" en la lista
-- 3. Haz clic en "Enable" o "Activar"
-- 4. Espera a que se habilite (puede tardar unos segundos)
-- 5. Luego ejecuta este script
--
-- Si intentas ejecutar este script sin PostGIS, fallará con el error:
-- "type geometry does not exist"
-- ============================================================================

-- Verificar que PostGIS está habilitado
-- Si PostGIS está habilitado desde el Dashboard, esta verificación solo confirma
DO $$
DECLARE
    v_postgis_exists BOOLEAN;
BEGIN
    -- Verificar que la extensión PostGIS existe
    SELECT EXISTS (
        SELECT 1 FROM pg_extension WHERE extname = 'postgis'
    ) INTO v_postgis_exists;
    
    -- Si PostGIS no está habilitado, intentar crearlo
    IF NOT v_postgis_exists THEN
        BEGIN
            CREATE EXTENSION IF NOT EXISTS postgis WITH SCHEMA public;
            RAISE NOTICE 'PostGIS habilitado correctamente.';
        ...
        END;
    ELSE
        RAISE NOTICE 'PostGIS ya está habilitado. Continuando...';
    END IF;
    
    -- No verificamos el tipo geometry explícitamente porque puede estar en diferentes schemas
    -- Si PostGIS está habilitado, el tipo debería estar disponible
END $$;

-- Configurar search_path (extensions debe estar incluido para PostGIS)
-- NOTA: En Supabase, PostGIS crea los tipos en el schema "extensions"
SET search_path TO public, extensions, core, catalog, orders, reviews, communication, commerce, social;

-- Verificar que podemos usar el tipo geometry y funciones de PostGIS
DO $$
BEGIN
    -- Intentar usar el tipo geometry (está en el schema extensions)
    PERFORM 'POINT(0 0)'::extensions.geometry;
    RAISE NOTICE 'Tipo geometry verificado correctamente (en schema extensions)';
    
    -- Intentar usar funciones de PostGIS (deberían estar en extensions)
    PERFORM extensions.ST_MakePoint(0, 0);
    RAISE NOTICE 'Funciones de PostGIS verificadas correctamente';
EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'PostGIS no está correctamente configurado. Error: %', SQLERRM;
END $$;
...
CREATE TABLE IF NOT EXISTS core.service_regions (
    ...
    -- NOTA: En Supabase, PostGIS crea el tipo geometry en el schema "extensions"
    coverage_area extensions.geometry(POLYGON, 4326) NOT NULL, -- SRID 4326 = WGS84
