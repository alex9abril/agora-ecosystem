-- ============================================================================
-- AGORA ECOSYSTEM - Upsert: Template global password_recovery
-- ============================================================================
-- Descripción: Inserta o actualiza el template de correo para recuperación de
--              contraseña en communication.email_templates. Sin esta fila, el
--              backend no envía el correo (EmailService devuelve "template no
--              encontrado" para trigger_type password_recovery).
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f3f4f6;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #f3f4f6; padding: 0; margin: 0;">
    ...
    ARRAY['user_name', 'recovery_link', 'business_name', 'business_logo']::text[],
    TRUE,
    TRUE
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'El template password_recovery no existe tras el upsert';
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM communication.email_templates
        WHERE trigger_type = 'password_recovery'
        ...
        RAISE EXCEPTION 'El template password_recovery no contiene recovery_link';
    END IF;
    RAISE NOTICE 'OK: template password_recovery listo para EmailService';
END $$;

-- ============================================================================
-- NOTAS
-- ============================================================================
-- - Requisito: tabla communication.email_templates ya creada (migration_email_templates).
-- - Idempotente: puedes ejecutar este archivo varias veces.
-- - Tras ejecutar, prueba de nuevo POST /api/auth/password/reset desde el front.
-- - Verificación manual: SELECT * FROM communication.get_email_template(
--     'password_recovery', NULL::uuid, NULL::uuid);
-- ============================================================================
